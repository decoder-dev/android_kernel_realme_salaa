		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (r < 10 || r > MAX_LUT_SCALE) return -EINVAL;
	if (g < 10 || g > MAX_LUT_SCALE) return -EINVAL;
	if (b < 10 || b > MAX_LUT_SCALE) return -EINVAL;
...
static void mtk_disp_rgb_work(struct work_struct *work) {
	...
	gamma->hw_id = 0;
	for (i = 0; i < 512; i++) {
		gammutR = i * (r-10) / PROGRESSION_SCALE;
		gammutG = i * (g-10) / PROGRESSION_SCALE;
		gammutB = i * (b-10) / PROGRESSION_SCALE;
